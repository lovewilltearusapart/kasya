$(document).ready(function() {
	...
	setTimeout(function() {
			$('#login-input').animate({'opacity': '1'}, 1500);
	}, 2000);
	...
	$('#login-input').keypress(function(e) {
		if(e.which === 13 && $(this).val() != '') {
			...
			window.setInterval(function() {
				if (won === 0) {
					if (help === 1) {
						$('#objectives #help').css({'color': 'tomato'});
						$('#objectives #list').animate({'opacity': '1'}, 1000);
						if (list === 1) {
							$('#objectives #list').css({'color': 'tomato'});
							$('#objectives #connect').animate({'opacity': '1'}, 1000);
							if (connect === 1) {
								$('#objectives #connect').css({'color': 'tomato'});
								$('#objectives #trojan').animate({'opacity': '1'}, 1000);
								if (trojan === 1) {
									$('#objectives #trojan').css({'color': 'tomato'});
				...
				if (disconnect === 1) {
					$('#objectives #disconnect').css({'color': 'tomato'});
				}
			...
			setTimeout(function(){
				$('span').remove();
				$('#terminal').append('<br><div>Welcome to GREENSYS [version 1.0.0].<br>(c) Copyright 2014 HACorp Corporation. All rights reserved.<br><br>' + player_name + ' authenticated.<br>GREENSYS ready for use.<br>Use the help command for a list of commands.<br><br><span id="initial-root">$</span>&nbsp;&nbsp;&nbsp;<span id="blinking">_</span></div>');
			}, 6500);
					...
					if(e.which === 13 && $(this).val() != '' && able === 1) {
						...
						switch( $(this).val() ) {
							...
							case 'connect nearest.phone.relay':
								if (connect === 0) {
									$('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br><p2>Connecting...</p2><br><p2>Connected.</p2><br><p2>You have 1 minute before you get traced down.</p2><span id="blinking">_</span></div>');
									connect = 1;
								} else {
							...
							case 'send trojan':
								if (connect === 1) {
									$('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br><p2>Sending Trojan.Vaklik.BBB...</p2><br><p2>Trojan.Vaklik.BBB sent.</p2><span id="blinking">_</span></div>');
									trojan = 1;
								} else {
							...
							case 'exit':
								...
								break;
							default:
								$('#terminal').append('<div>$&nbsp;&nbsp;&nbsp;' + $(this).val() + '<br><p1>Command not recognized. Type in help for a list of commands.</p1><span id="blinking">_</span></div>');
						}
						$('#terminal').append('<br>');
